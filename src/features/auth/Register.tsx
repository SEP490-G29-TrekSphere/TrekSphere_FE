...
export default function Register() {
  ...
  const onSubmit = async (data: RegisterFormValues) => {
    try {
      ...
      setUser({ id: '1', name: data.fullName });
  ...
  return (
    <AuthLayout
      ...
      footerLink={{ label: 'Đăng nhập', to: '/login' }}
      ...
      <FormProvider {...methods}>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <button
            ...
            className="w-full h-12 rounded-full text-white font-semibold text-sm flex items-center justify-center transition-opacity hover:opacity-90 disabled:opacity-60"
