...
export function GroupBudgetTab({
  ...
  // 2. Actual Expenses Data (Chỉ fetch khi không phải outsider)
  const { data: expensesData, isLoading: isLoadingExpenses } = useGroupExpenses(groupId, 0, 50, {
    enabled: !isOutsider,
  });

  // 3. Settlement Data (Chỉ fetch khi không phải outsider)
  const {
            ...
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-border">
                {isLoadingExpenses ? (
                  ...
                  <tr>
                    <td colSpan={5} className="p-8 text-center text-xs text-muted-foreground">
                      Chưa có hóa đơn thực tế nào. Bấm &quot;Nhập Hóa Đơn&quot; để thêm khoản đã
                      ứng.
                    </td>
                  </tr>
                ) : (
          ...
          <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/5 p-4 sm:p-5 space-y-4">
            ...
            <div className="text-xs text-foreground leading-relaxed space-y-1.5 border-t border-emerald-500/20 pt-3">
              ...
              <div className="flex items-center gap-1.5 text-emerald-800 dark:text-emerald-300 font-bold">
                ...
                Đã tối ưu hóa còn {displaySettlements.length} giao dịch trực tiếp giữa các thành
                viên:
              </div>
            </div>
                          ...
                          {persistedSt && (
                            <div className="flex items-center gap-3 text-[11px] text-muted-foreground">
                              <span className="flex items-center gap-1">
                                <Clock className="h-3 w-3" /> Hạn thanh toán: Sau khi kết thúc
                                chuyến đi
                              </span>
                            </div>
                          )}
